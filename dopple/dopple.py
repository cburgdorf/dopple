...
class UnixSocketConnector(object):
    ...
    def sendall(self, data: bytes) -> None:
        try:
            return self.socket().sendall(data)  # type: ignore
        except OSError as ex:
...
class NamedPipeConnector(object):
    ...
    def __init__(self, ipc_path: str) -> None:
        try:
            self.handle = win32file.CreateFile(  # type: ignore
                ipc_path, win32file.GENERIC_READ | win32file.GENERIC_WRITE,  # type: ignore
                0, None, win32file.OPEN_EXISTING, 0, None)  # type: ignore
    ...
    def recv(self, max_length: int) -> Any:
        (err, data) = win32file.ReadFile(self.handle, max_length)  # type: ignore
        if err:
            raise IOError(err)
        return data

    def sendall(self, data: bytes) -> win32file.WriteFile:  # type: ignore
        return win32file.WriteFile(self.handle, data)  # type: ignore
